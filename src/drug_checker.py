...
def full_interaction_check(prev_meds, new_med):
    return "\n\n".join([f"{new_med} + {med}: {check_drug_pair(med.strip(), new_med)}" for med in prev_meds])

if __name__ == "__main__":
    t = time.time()
    prev = ["warfarin", "metformin"]
    new = "aspirin"
    print(" MedGemma RAG (Server Live!)")
    print("=" * 50)
    print(full_interaction_check(prev, new))
